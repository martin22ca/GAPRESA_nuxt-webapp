<template>
    <ModalMC :modalOpen="modalControl.status" :toggleModal="toggleModal" :modalText="modalControl.text"
    ...
    <div class="flex h-screen bg-base-300 ">
        ...
        <div class="w-2/3 flex items-center justify-center card infoAnim">
            <div v-if="failedOnce" class="infoAnim self-center badge p-4 py-8 badge-neutral text-center border-2 border-accent">
            ...
            <form class="flex flex-col w-5/6 p-6 bg-base-100 shadow-md rounded-md mt-8" @submit.prevent="submit">
                ...
                <GenericMCInput text-label="Username" :text-error="user_name.errorMessage.value"
                    text-icon="material-symbols:account-circle">
                    <input v-model="user_name.value.value" type="text" class="input input-bordered w-full" />
                </GenericMCInput>
                <GenericMCInput text-label="Contraseña" :text-error="user_pass.errorMessage.value" text-icon="gg:password">
...
const submit = handleSubmit(async (values) => {
    ...
    if (data.value.success) {
        ...
        store.name = data.value.full_name
        store.id = data.value.user_id
        ...
        modalControl.value.text = data.value.error
