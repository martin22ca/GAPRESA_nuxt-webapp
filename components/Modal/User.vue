<template>
    <form @submit.prevent="submit">
        <div class="header flex flex-row gap-4 bg-neutral px-4 rounded-xl">
            ...
            <GenericMCInput textIcon="material-symbols:account-circle" textLabel="Fecha Inicio"
                :textError="start_date.errorMessage.value">
                <input v-model="start_date.value.value" type="date" class="input input-bordered w-full"
                    :disabled="update" />
            </GenericMCInput>
            <GenericMCInput v-if="update" class="" textIcon="material-symbols:account-circle" textLabel="Fecha Fin"
        ...
        <div class="bg-neutral px-4 rounded-xl">
            <div class="flex flex-row gap-4 ">
                <GenericMCInput class="basis-1/3" textIcon="material-symbols:account-circle" textLabel="Username"
                    :textError="user_name.errorMessage.value">
                    <input v-model="user_name.value.value" class="input input-bordered w-full" />
                </GenericMCInput>
                <GenericMCInput class="basis-1/3" textIcon="icon-park-outline:edit-name" textLabel="Nombre"
                    :textError="first_name.errorMessage.value">
                    <input v-model="first_name.value.value" class="input input-bordered w-full" />
                </GenericMCInput>
                <GenericMCInput class="basis-1/3" textIcon="icon-park-outline:edit-name" textLabel="Apellido"
                    :textError="last_name.errorMessage.value">
                    <input v-model="last_name.value.value" class="input input-bordered w-full" />
                </GenericMCInput>
            </div>
            <div class="flex flex-row gap-4">
                <GenericMCInput class="basis-1/3" textIcon="heroicons:identification-20-solid" textLabel="Cuil"
                    :textError="cuil.errorMessage.value">
                    <input v-model="formattedCuil" type="number" class="input input-bordered w-full" />
                </GenericMCInput>
                <GenericMCInput class="basis-2/3" textIcon="material-symbols:location-on-outline" textLabel="Direccion"
                    :textError="address.errorMessage.value">
                    <input v-model="address.value.value" class="input input-bordered w-full" />
                </GenericMCInput>
            </div>
            <div class="flex flex-row gap-4">
                <GenericMCInput class="basis-1/2" textIcon="material-symbols:call-log" textLabel="Telefono ALT"
            ...
            <div class="flex flex-row gap-4">
                <GenericMCInput class="basis-1/2" textIcon="ic:sharp-email" textLabel="Email Coorporativo"
                    :textError="email_corp.errorMessage.value">
                    <input v-model="email_corp.value.value" class="input input-bordered w-full" />
                </GenericMCInput>
                <GenericMCInput class="basis-1/2" textIcon="ic:sharp-email" textLabel="Email Personal"
                    :textError="email_personal.errorMessage.value">
                    <input v-model="email_personal.value.value" class="input input-bordered w-full" />
                </GenericMCInput>
            </div>
            <div class="flex flex-row gap-4">
                <GenericMCInput class="basis-1/2" textIcon="gg:password" textLabel="Contraseña"
                ...
                <GenericMCInput class="basis-1/2" textIcon="gg:password" textLabel="Confirmar Contraseña"
